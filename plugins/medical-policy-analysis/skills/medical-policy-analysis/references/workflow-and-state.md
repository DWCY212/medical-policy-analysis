...
- 首次运行：用户上传资料后、读取政策正文和联网分析前，检查是否已明确选择模式。未选择时原样提示：
...
- 用户在上传资料的同一条消息中已明确选择模式时，直接采用，不重复询问。
- 未选择模式前只确认资料已收到、识别文件类型和保存位置；不分析正文、不联网检索、不创建状态文件或步骤产物。
- 手动逐步生成：每步完成后说明产物路径、联网核验状态、待确认项和下一步动作，然后暂停。用户可修改当前步；只有收到“继续”后才推进。
- 自动逐步生成：每步仍单独落盘、自检并更新状态；只有完成条件满足后才连续推进。
